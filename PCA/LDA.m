...
tempMean = zeros(112,92);
tempSum = zeros(112,92);
classMean = zeros(112,92,6);
for i = 1:1:6
    for j = 1:1:8
        ...
        tempSum = tempSum + x2;
        classMean(:,:,i) = classMean(:,:,i) + x2;
    end
    classMean(:,:,i) = classMean(:,:,i) / 8;  % 각 클래스의 평균 얼굴 구하기
end

% 전체 training vector의 평균 얼굴 구하기
m = tempSum/48;
...
% Normalization (클래스내)
tempDevi = zeros(112,92,48);
tempVec2 = zeros(112*92,48);
for i = 1:1:6
    for j = 1:1:8
        tempDevi(:,:,8*(i-1)+j) = trainingVec(:,:,8*(i-1)+j) - classMean(:,:,i);  
        tempVec2(:,8*(i-1)+j) = reshape(tempDevi(:,:,8*(i-1)+j),[112*92 1]);
    end
end
D_intra = tempVec2 * transpose(tempVec2);

% Normalization (클래스간)
tempDevi2 = zeros(112,92,6);
tempVec3 = zeros(112*92,6);
for i = 1:1:6
    tempDevi2(:,:,i) = classMean(:,:,i) - m;
    tempVec3(:,i) = reshape(tempDevi2(:,:,i),[112*92 1]);
end
D_inter = tempVec3 * transpose(tempVec3);

% 공분산 행렬 구하기
convariance = pinv(D_intra) * D_inter;
